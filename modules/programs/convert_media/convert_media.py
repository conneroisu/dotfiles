...
SUPPORTED_VIDEO_FORMATS = {
    'mp4', 'avi', 'mov', 'mkv', 'webm', 'flv', 'wmv', 'm4v', '3gp', 'ogv', 'gif'
}

SUPPORTED_AUDIO_FORMATS = {
    'mp3', 'wav', 'flac', 'aac', 'ogg', 'wma', 'm4a', 'opus', 'ac3', 'aiff'
}
...
def validate_input(input_path: Path) -> str:
    ...
    if extension in SUPPORTED_VIDEO_FORMATS:
        ...
        print(f"Warning: Format '{extension}' may not be supported")
        return 'unknown'


def get_quality_settings(quality: str, media_type: str) -> Dict[str, str]:
    """Get quality settings based on preset."""
    settings = {}
    
    if media_type == 'video':
        if quality == "low":
            settings.update({"crf": "28", "preset": "fast"})
        elif quality == "medium":
            settings.update({"crf": "23", "preset": "medium"})
        elif quality == "high":
            settings.update({"crf": "18", "preset": "slow"})
        elif quality == "lossless":
            settings.update({"crf": "0", "preset": "medium"})
    else:  # audio
        if quality == "low":
...
def build_gif_with_palette(args: argparse.Namespace) -> bool:
    ...
    try:
        ...
        if not args.verbose:
            palette_cmd.extend(["-loglevel", "error"])
        
        palette_cmd.extend(["-i", str(args.input_path)])
        
        if args.start_time:
            palette_cmd.extend(["-ss", args.start_time])
        if args.duration:
            palette_cmd.extend(["-t", args.duration])
        
        # Generate palette
        palette_filter = "palettegen=reserve_transparent=0"
        if args.resolution:
            palette_filter = f"scale={args.resolution}:flags=lanczos,{palette_filter}"
        
        palette_cmd.extend(["-vf", palette_filter, str(palette_path)])
        ...
        gif_cmd.extend(["-i", str(args.input_path), "-i", str(palette_path)])
        
        if args.start_time:
            gif_cmd.extend(["-ss", args.start_time])
        if args.duration:
        ...
        gif_cmd.extend(["-filter_complex", f"[0:v]{gif_filter}[v];[v][1:v]paletteuse"])
        gif_cmd.extend(["-loop", str(args.gif_loop)])
        ...
        subprocess.run(palette_cmd, check=True, capture_output=not args.verbose)
        
        # Execute GIF creation
        subprocess.run(gif_cmd, check=True, capture_output=not args.verbose)
...
def build_ffmpeg_command(args: argparse.Namespace, input_type: str) -> List[str]:
    ...
    # Get output format
    output_ext = args.output_path.suffix.lower().lstrip('.')
    output_type = 'video' if output_ext in SUPPORTED_VIDEO_FORMATS else 'audio'
    
    # Quality settings
    quality_settings = get_quality_settings(args.quality, output_type)
    ...
    if args.codec:
        ...
        if output_type == 'video' and output_ext in VIDEO_CODECS:
            cmd.extend(["-vcodec", VIDEO_CODECS[output_ext]])
        elif output_type == 'audio' and output_ext in AUDIO_CODECS:
            cmd.extend(["-acodec", AUDIO_CODECS[output_ext]])
    
    # Video-specific settings
    if output_type == 'video' and not args.audio_only:
        if args.resolution:
    ...
    if args.bitrate:
        ...
        cmd.extend(["-b:a", quality_settings["ab"]])
    ...
    if output_ext == 'gif' and not args.gif_palette:
...
def get_media_info(file_path: Path) -> Dict[str, str]:
    ...
    try:
        ...
        info = {}
        if "format" in data:
            info["duration"] = data["format"].get("duration", "Unknown")
            info["size"] = data["format"].get("size", "Unknown")
        ...
        for stream in data.get("streams", []):
            if stream["codec_type"] == "video":
                ...
                info["channels"] = stream.get("channels", "Unknown")
...
def print_file_info(input_path: Path, output_path: Path) -> None:
...
def main() -> None:
    ...
    if args.audio_only and args.video_only:
        print("Error: Cannot specify both --audio-only and --video-only")
        sys.exit(1)
    ...
    if args.output_path.exists() and not args.overwrite:
        response = input(f"Output file {args.output_path} exists. Overwrite? [y/N]: ")
    ...
    if output_ext == 'gif' and args.gif_palette:
        if args.dry_run:
            print("GIF with palette generation (two-pass encoding)")
            print("This would execute palette generation and GIF creation commands")
            return
        ...
        if success:
            print(f"Successfully converted {args.input_path.name} → {args.output_path.name}")
            print("Method: GIF with optimized palette")
            print_file_info(args.input_path, args.output_path)
        else:
            sys.exit(1)
        return
