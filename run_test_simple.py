...
from run import (
    ...
    create_parser, RE_VERSION, RE_HASH, PACKAGE_MAPPINGS, SKIP_PACKAGES
)
...
class TestBasicFunctionality(unittest.TestCase):
    ...
    def test_config_creation(self):
        ...
        self.assertEqual(config.shell_name, "default")
    ...
    def test_data_creation(self):
        ...
        self.assertEqual(len(data.build_inputs), 2)
        self.assertEqual(data.system, "x86_64-linux")
        
        # Test to_dict conversion
        data_dict = data.to_dict()
        self.assertIsInstance(data_dict, dict)
        self.assertEqual(data_dict['system'], "x86_64-linux")
    ...
    def test_drv_to_pkgname_basic(self):
        ...
        test_cases = [
            ("", None),
            ("/nix/store/abcdefgh12345678abcdefgh12345678-hello-1.0", "hello"),
            ("/nix/store/abcdefgh12345678abcdefgh12345678-python3-3.11.0", "python3"),
            ("/nix/store/abcdefgh12345678abcdefgh12345678-dx", None),  # Should be skipped
            ("/nix/store/abcdefgh12345678abcdefgh12345678-rust-default", "rustc"),  # Mapping
        ]
    ...
    def test_uniq_preserve_order(self):
        ...
        input_seq = ["a", "b", None, "a", "c", None, "b"]
        result = extractor.uniq_preserve_order(input_seq)
        
        self.assertEqual(result, ["a", "b", "c"])
        
        # Test with empty list
        self.assertEqual(extractor.uniq_preserve_order([]), [])
        
        # Test with all None
        self.assertEqual(extractor.uniq_preserve_order([None, None]), [])
    
    def test_regex_patterns(self):
        """Test regex patterns work as expected."""
        # Test version regex
        self.assertIsNotNone(RE_VERSION.match("1.0"))
        self.assertIsNotNone(RE_VERSION.match("123abc"))
        self.assertIsNone(RE_VERSION.match("abc123"))
        self.assertIsNone(RE_VERSION.match(""))
        
        # Test hash regex - should match exactly 32 lowercase hex chars
        self.assertIsNotNone(RE_HASH.fullmatch("abcdef1234567890abcdef1234567890"))
        self.assertIsNone(RE_HASH.fullmatch("ABCDEF1234567890ABCDEF1234567890"))  # Uppercase
        self.assertIsNone(RE_HASH.fullmatch("abcdef123"))  # Too short
        self.assertIsNone(RE_HASH.fullmatch("abcdef1234567890abcdef1234567890x"))  # Too long
        self.assertIsNone(RE_HASH.fullmatch(""))  # Empty
    
    def test_package_mappings(self):
        """Test package mappings are valid."""
        self.assertIsInstance(PACKAGE_MAPPINGS, dict)
        self.assertGreater(len(PACKAGE_MAPPINGS), 0)
        
        # Test some known mappings
        self.assertIn("rust-default", PACKAGE_MAPPINGS)
        self.assertEqual(PACKAGE_MAPPINGS["rust-default"], "rustc")
    ...
    def test_argument_parser(self):
        ...
        self.assertEqual(args.flake_path, "test-flake")
        self.assertEqual(args.shell_name, "default")
        self.assertFalse(args.verbose)
        
        # Test with options
        args = parser.parse_args(["--verbose", "--shell-name", "custom", "test-flake"])
        self.assertTrue(args.verbose)
        self.assertEqual(args.shell_name, "custom")
    ...
    def test_format_json_output(self):
        ...
        output = extractor.format_json_output(data)
        
        # Should be valid JSON
        import json
        parsed = json.loads(output)
        self.assertEqual(parsed['system'], "x86_64-linux")
        self.assertEqual(parsed['build_inputs'], ["test"])
    ...
    def test_drv_to_pkgname_with_unusual_paths(self):
        ...
        for path in unusual_paths:
            with self.subTest(path=path):
                # Should not crash
                result = extractor.drv_to_pkgname(path)
    ...
    def test_uniq_preserve_order_stress(self):
        ...
        for i in range(1000):
            if i % 10 == 0:
                ...
                large_list.append(f"item-{i % 50}")  # Create duplicates
        
        result = extractor.uniq_preserve_order(large_list)
        
        # Should have unique items only, no None values
        self.assertEqual(len(set(result)), len(result))  # All unique
        self.assertNotIn(None, result)  # No None values
        self.assertLessEqual(len(result), 50)  # At most 50 unique items


if __name__ == '__main__':
    print("Running basic verification tests for run.py...")
