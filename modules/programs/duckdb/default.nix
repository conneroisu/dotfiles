{
  ...
  program = pkgs.stdenv.mkDerivation (finalAttrs: {
    pname = "duckdb";
    ...
    buildInputs =
      [pkgs.openssl] ++ pkgs.lib.optionals false [pkgs.openjdk11] ++ pkgs.lib.optionals false [pkgs.unixODBC];

    cmakeFlags =
      [
        "-DDUCKDB_EXTENSION_CONFIGS=${finalAttrs.src}/.github/config/in_tree_extensions.cmake"
        "-DBUILD_ODBC_DRIVER=${enableFeature false}"
        "-DJDBC_DRIVER=${enableFeature false}"
        "-DOVERRIDE_GIT_DESCRIBE=v${finalAttrs.version}-0-g${finalAttrs.rev}"
      ]
      ++ pkgs.lib.optionals finalAttrs.doInstallCheck [
    ...
    installCheckPhase = let
      ...
      LD_LIBRARY_PATH = pkgs.lib.optionalString pkgs.stdenv.hostPlatform.isDarwin "DY" + "LD_LIBRARY_PATH";
    in ''
