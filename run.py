...
RE_VERSION = re.compile(
    r"^\d"
)  # first component that *starts* with a digit
RE_HASH = re.compile(
    r"^[0-9a-z]{32}$"
)  # /nix/store/<hash>-...
...
class DevShellExtractor:
        ...
        *,
        ...
        try:
            ...
            if (
                ...
                raise NixError(
                    f"Platform incompatibility: {platform_error}"
                )
            elif (
                "No commit found for SHA"
                in stderr_text
                and "github.com" in stderr_text
            ):
                raise NixError(
                ...
                raise NixError(
                    f"Nix evaluation failed: {stderr_text}"
                )
    ...
    def get_current_system(self) -> str:
        ...
        try:
            ...
            raise NixError(
                f"Failed to detect current system: {e}"
            )
    ...
    def uniq_preserve_order(
        self, seq: Iterable[Optional[str]]
    ) -> List[str]:
    ...
    def discover_devshells(
        self, flake: str
    ) -> Dict[str, List[str]]:
        """Discover available devShells in a flake."""
        try:
            devshells_json = self.nix_eval_json(
                f"{flake}#devShells or {{}}"
            )
    ...
    def find_devshell_attr(
        ...
        attr_base = (
            f'devShells."{system}".{shell_name}'
        )
        try:
            self.logger.debug(
                f"Trying: {flake}#{attr_base}"
            )
            self.nix_eval_json(
                f"{flake}#{attr_base}"
            )
        ...
        if shell_name == "default":
            ...
            try:
                self.logger.debug(
                    f"Trying legacy: {flake}#{attr_base}"
                )
                self.nix_eval_json(
                    f"{flake}#{attr_base}"
                )
    ...
    def extract_devshell_data(
        ...
        attr_base = self.find_devshell_attr(
            flake, system, shell_name
        )
        ...
        try:
            self.logger.debug(
                "Extracting buildInputs..."
            )
            build_inputs_json = self.nix_eval_json(
                f"{flake}#{attr_base}.buildInputs"
            )
            attrs["buildInputs"] = json.loads(
                build_inputs_json
            )
        except NixError:
            attrs["buildInputs"] = []
        ...
        if self.config.include_shell_hook:
            try:
                self.logger.debug(
                    "Extracting shellHook..."
                )
                shell_hook_json = self.nix_eval_json(
                    f"{flake}#{attr_base}.shellHook"
                )
                shell_hook = json.loads(
                    shell_hook_json
                )
            except NixError:
                shell_hook = ""
        ...
        all_build_inputs = [
            self.drv_to_pkgname(d)
            for d in attrs["buildInputs"]
        ]
        all_native_inputs = [
            self.drv_to_pkgname(d)
            for d in attrs["nativeBuildInputs"]
        ]
        all_prop_inputs = [
            self.drv_to_pkgname(d)
            for d in attrs[
        ...
        return DevShellData(
            build_inputs=self.uniq_preserve_order(
                all_build_inputs
            ),
            native_build_inputs=self.uniq_preserve_order(
                all_native_inputs
            ),
            propagated_build_inputs=self.uniq_preserve_order(
                all_prop_inputs
            ),
    ...
    def format_nix_output(
        ...
        if self.config.separate_inputs:
          ...
          {prop_inputs_str}"""
            ...
            all_inputs = self.uniq_preserve_order(
                all_inputs
            )
            inputs_section = f"""
    ...
    def _format_package_list(
        self, packages: List[str], attr_name: str
    ) -> str:
    ...
    def extract_and_format(self) -> str:
        ...
        self.logger.info(
            f"Analyzing flake: {flake_path}"
        )
        self.logger.info(
            f"Target system: {system}"
        )
        self.logger.info(
            f"DevShell name: {self.config.shell_name}"
        )
        ...
        self.logger.info(
            f"Successfully extracted {total_packages} packages"
        )
