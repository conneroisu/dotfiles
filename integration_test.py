...
class IntegrationTester:
    ...
    def test_real_flake_extraction(self):
                ...
                if result.returncode == 0:
                    ...
                    try:
                        ...
                        self.results['real_flake_extraction'].append({
                            ...
                            'success': True,
                            'has_build_inputs': len(data.get('build_inputs', [])) > 0,
        ...
        successful = len([r for r in self.results['real_flake_extraction'] if r.get('success', False)])
        print(f"   ✅ Successfully extracted from {successful}/{len(self.results['real_flake_extraction'])} flakes")
    ...
    def test_command_line_execution(self):
        ...
        test_cases = [
            ...
            {
                'args': ['.', '--separate-inputs'],
                'description': 'Separate input types'
            },
            {
                'args': ['.', '--shell-name', 'default'],
                'description': 'Specific shell name'
            }
        ]
        
        with tempfile.NamedTemporaryFile(mode='w', suffix='.nix', delete=False) as temp_file:
            temp_output = temp_file.name
        
        try:
            for test_case in test_cases:
                try:
                    args = [sys.executable, str(self.script_path)] + test_case['args'] + ['-o', temp_output]
                    result = subprocess.run(args, capture_output=True, text=True, timeout=20)
                    
                    # Check if output file was created (for cases with -o flag)
                    output_created = Path(temp_output).exists() and Path(temp_output).stat().st_size > 0
        ...
        successful = len([r for r in self.results['command_line_execution'] if r.get('success', False)])
    ...
    def test_output_format_validation(self):
            ...
            try:
                ...
                if result.returncode == 0 and result.stdout:
                    ...
                    if format_type == 'json':
                        ...
                        format_valid = 'pkgs.mkShell' in result.stdout and '{' in result.stdout
                        validation_msg = "Valid Nix format" if format_valid else "Invalid Nix format"
                    else:
                        format_valid = True
        ...
        valid_formats = len([r for r in self.results['output_format_validation'] 
    ...
    def test_error_scenario_handling(self):
        ...
        error_scenarios = [
            ...
            {
                'args': ['.', '--shell-name', 'nonexistent'],
                'description': 'Non-existent shell name',
                'expected_failure': True
            },
            {
                'args': ['.', '--output', '/root/forbidden.nix'],
        ...
        for scenario in error_scenarios:
            try:
                args = [sys.executable, str(self.script_path)] + scenario['args']
                result = subprocess.run(args, capture_output=True, text=True, timeout=10)
                
                # For expected failures, non-zero return code is success
                scenario_handled = (result.returncode != 0) == scenario['expected_failure']
                ...
                self.results['error_scenario_handling'].append({
                    ...
                    'has_error_message': len(result.stderr) > 0 if result.stderr else False
                })
        ...
        handled = len([r for r in self.results['error_scenario_handling'] if r.get('scenario_handled', False)])
        print(f"   ✅ {handled}/{len(error_scenarios)} error scenarios handled correctly")
    ...
    def test_template_compatibility(self):
                ...
                if result.returncode == 0:
                    try:
                        ...
                        self.results['template_compatibility'].append({
                            ...
                            'success': True,
                            'packages_found': len(data.get('build_inputs', [])) + 
                                            len(data.get('native_build_inputs', [])),
                            'has_shell_hook': bool(data.get('shell_hook', '').strip())
                        })
                    except json.JSONDecodeError:
                        self.results['template_compatibility'].append({
                            'template': template_path.name,
        ...
        compatible = len([r for r in self.results['template_compatibility'] if r.get('success', False)])
    ...
    def generate_report(self):
        ...
        for category, results in self.results.items():
            ...
            if category == 'error_scenario_handling':
                ...
                passed = len([r for r in results if r.get('scenario_handled', False)])
            else:
