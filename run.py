...
class DevShellExtractor:
    ...
    def nix_eval_json(self, expr: str, *, impure: bool = None, use_expr: bool = False) -> str:
        ...
        try:
            ...
            elif "No commit found for SHA" in stderr_text and "github.com" in stderr_text:
                raise NixError(f"Invalid remote flake reference - may be treating local path as remote URL: {stderr_text}")
            elif "is not tracked by Git" in stderr_text:
                raise NixError(f"Git tracking issue: Files need to be added to git. Use 'git add' to track the flake files.")
            else:
    ...
    def find_devshell_attr(self, flake: str, system: str, shell_name: str) -> str:
        ...
        raise NixError(f"No devShell '{shell_name}' found for system '{system}' in {flake}")

    def extract_devshell_data(self, flake: str, system: str, shell_name: str) -> DevShellData:
        ...
        try:
            ...
            native_inputs_json = self.nix_eval_json(f"{flake}#{attr_base}.nativeBuildInputs")
        ...
        try:
            ...
            prop_inputs_json = self.nix_eval_json(f"{flake}#{attr_base}.propagatedBuildInputs")
        ...
        all_prop_inputs = [self.drv_to_pkgname(d) for d in attrs["propagatedBuildInputs"]]
    ...
    def format_nix_output(self, data: DevShellData) -> str:
        ...
        if self.config.separate_inputs:
            ...
            build_inputs_str = self._format_package_list(data.build_inputs, "buildInputs")
            native_inputs_str = self._format_package_list(data.native_build_inputs, "nativeBuildInputs") 
            prop_inputs_str = self._format_package_list(data.propagated_build_inputs, "propagatedBuildInputs")
          ...
          {prop_inputs_str}"""
            ...
            # Combine all inputs
            all_inputs = data.build_inputs + data.native_build_inputs + data.propagated_build_inputs
    ...
    def format_yaml_output(self, data: DevShellData) -> str:
        ...
        if not YAML_AVAILABLE:
            raise NixError("YAML support not available. Install PyYAML: pip install PyYAML")
    ...
    def extract_and_format(self) -> str:
        ...
        if not flake_path.startswith(('./', '/', 'git+', 'github:', 'gitlab:', 'sourcehut:', 'file:')):
        ...
        # Extract data
        data = self.extract_devshell_data(flake_path, system, self.config.shell_name)
        
        total_packages = len(data.build_inputs) + len(data.native_build_inputs) + len(data.propagated_build_inputs)
...
Examples:
    ...
    parser.add_argument("flake_path", help="Path or URL to the flake")
    parser.add_argument("system", nargs="?", help="Target system (auto-detected if not specified)")
    ...
    parser.add_argument("--shell-name", default="default", help="Specify devShell name (default: default)")
    format_choices = ["nix", "json"]
    if YAML_AVAILABLE:
        format_choices.append("yaml")
    parser.add_argument("--format", choices=format_choices, default="nix", 
                       help="Output format (default: nix)")
    parser.add_argument("--no-shell-hook", action="store_true", help="Exclude shellHook from output")
    parser.add_argument("--separate-inputs", action="store_true", 
                       help="Separate buildInputs, nativeBuildInputs, etc.")
    parser.add_argument("--impure", action="store_true", 
    ...
    try:
        ...
        if config.output_file:
            with open(config.output_file, 'w') as f:
                f.write(output)
            if not config.quiet:
                print(f"Output written to {config.output_file}", file=sys.stderr)
        else:
            print(output)
