...
from run import (
    DevShellConfig, DevShellData, DevShellExtractor, NixError,
    create_parser, main, PACKAGE_MAPPINGS, SKIP_PACKAGES,
    RE_VERSION, RE_HASH
)
...
class FuzzTestGenerator:
    ...
    def derivation_paths() -> Generator[str, None, None]:
        ...
        paths = [
            ...
            "package",  # No path
            "/nix/store/abcdefgh12345678abcdefgh12345678-" + "a" * 200,  # Very long name
...
class TestDevShellConfig(unittest.TestCase):
    ...
    def test_config_creation_with_fuzz_inputs(self):
...
class TestDevShellData(unittest.TestCase):
    ...
    def test_data_creation_with_fuzz_lists(self):
        ...
        for _ in range(100):
            build_inputs = [FuzzTestGenerator.random_string() for _ in range(random.randint(0, 20))]
            native_inputs = [FuzzTestGenerator.random_string() for _ in range(random.randint(0, 20))]
            prop_inputs = [FuzzTestGenerator.random_string() for _ in range(random.randint(0, 20))]
            ...
            self.assertEqual(dict_result['build_inputs'], build_inputs)
...
class TestDevShellExtractor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = DevShellConfig(flake_path=".", verbose=False, quiet=True)
        self.extractor = DevShellExtractor(self.config)
    
    def test_nix_eval_json_with_malformed_expressions(self):
    ...
    def test_drv_to_pkgname_edge_cases(self):
        ...
        test_cases = [
            ("", None),  # Empty input
            ("/nix/store/abcdefgh12345678abcdefgh12345678-dx", None),  # Should be skipped
            ("/nix/store/abcdefgh12345678abcdefgh12345678-lint", None),  # Should be skipped
            ("/nix/store/abcdefgh12345678abcdefgh12345678-rust-default-1.0", "rustc"),  # Mapping
            ("/nix/store/abcdefgh12345678abcdefgh12345678-go_1.20", "go"),  # Mapping
        ]
        
        for drv_path, expected in test_cases:
            with self.subTest(drv_path=drv_path, expected=expected):
                result = self.extractor.drv_to_pkgname(drv_path)
    ...
    def test_uniq_preserve_order_with_fuzz_inputs(self):
        ...
        for flake in FuzzTestGenerator.malformed_paths():
            for system in FuzzTestGenerator.system_strings():
                for shell_name in FuzzTestGenerator.shell_names():
                    with self.subTest(flake=repr(flake), system=repr(system), shell_name=repr(shell_name)):
                        with self.assertRaises(NixError):
                            self.extractor.find_devshell_attr(flake, system, shell_name)
    
    def test_format_nix_output_with_fuzz_data(self):
        """Test format_nix_output with fuzzed DevShellData."""
        for _ in range(50):
            # Generate random DevShellData
            data = DevShellData(
                build_inputs=[FuzzTestGenerator.random_string(20) for _ in range(random.randint(0, 10))],
                native_build_inputs=[FuzzTestGenerator.random_string(20) for _ in range(random.randint(0, 10))],
                propagated_build_inputs=[FuzzTestGenerator.random_string(20) for _ in range(random.randint(0, 10))],
    ...
    def test_format_json_output_with_fuzz_data(self):
        """Test format_json_output with fuzzed data."""
        for _ in range(20):
            data = DevShellData(
                build_inputs=[FuzzTestGenerator.random_string() for _ in range(random.randint(0, 5))],
                native_build_inputs=[],
                propagated_build_inputs=[],
                shell_hook=FuzzTestGenerator.random_string(),
                system=FuzzTestGenerator.random_string(),
    ...
    def test_format_yaml_output_availability(self):
        ...
        try:
            ...
            self.assertIn("YAML support not available", str(e))
...
class TestArgumentParsing(unittest.TestCase):
    ...
    def test_parser_with_malformed_arguments(self):
        ...
        malformed_args = [
            ...
            ["flake", "system", "extra"],  # Too many positional args
        ]
    ...
    def test_parser_with_edge_case_arguments(self):
        ...
        edge_case_args = [
            ...
            ["-q", "-v", "."],  # Conflicting flags with valid flake
        ]
    ...
    def test_parser_with_valid_fuzz_inputs(self):
        ...
        for _ in range(100):
            flake_path = FuzzTestGenerator.random_string(50, string.ascii_letters + "./")
            args = [flake_path]
            ...
            if random.choice([True, False]):
                args.extend(["--verbose"])
            if random.choice([True, False]):
                args.extend(["--shell-name", FuzzTestGenerator.random_string(20, string.ascii_letters)])
            if random.choice([True, False]):
                args.extend(["--format", random.choice(["nix", "json"])])
            
            with self.subTest(args=args):
                try:
                    parsed = self.parser.parse_args(args)
                    self.assertEqual(parsed.flake_path, flake_path)
                except SystemExit:
...
class TestRegexPatterns(unittest.TestCase):
    ...
    def test_version_regex_with_fuzz_strings(self):
...
class TestPackageMappings(unittest.TestCase):
    ...
    def test_package_mappings_consistency(self):
        ...
        for pattern, replacement in PACKAGE_MAPPINGS.items():
            with self.subTest(pattern=pattern, replacement=replacement):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            for invalid_path in invalid_paths:
                with self.subTest(path=repr(invalid_path)):
                    ...
                    self.assertEqual(config.output_file, invalid_path)
...
class TestConcurrencyAndStress(unittest.TestCase):
    ...
    def test_multiple_extractor_instances(self):
        ...
        configs = [
            DevShellConfig(flake_path=".", quiet=True)
            for _ in range(100)
        ]
        ...
        for i, extractor in enumerate(extractors):
            with self.subTest(instance=i):
                self.assertIsInstance(extractor.config, DevShellConfig)
                self.assertIsInstance(extractor.logger, logging.Logger)
