...
def test_should_ignore():
    ...
    assert should_ignore("test.pyc", [re.compile(r".*\.pyc")], []) is True
    assert should_ignore("test.py", [re.compile(r".*\.pyc")], []) is False
    
    # Ignore directories
    assert should_ignore("__pycache__/test.py", [], ["__pycache__"]) is True
    assert should_ignore("src/test.py", [], ["__pycache__"]) is False
    
    # Combined patterns
    assert should_ignore("__pycache__/test.pyc", [re.compile(r".*\.pyc")], ["__pycache__"]) is True
...
def test_get_real_path():
    ...
    with patch("pathlib.Path.resolve", side_effect=Exception("Path error")):
        with patch("os.path.abspath", return_value="/fallback/path"):
            result = get_real_path(test_path)
...
def test_find_files_basic(fake_filesystem, mock_args):
    ...
    fake_filesystem.create_file("/test/file1.py", contents="print('hello')")
    fake_filesystem.create_file("/test/file2.txt", contents="text content")
    fake_filesystem.create_dir("/test/subdir")
    fake_filesystem.create_file("/test/subdir/file3.py", contents="print('world')")
...
def test_find_files_recursive(fake_filesystem, mock_args):
    ...
    fake_filesystem.create_file("/test/file1.py", contents="print('hello')")
    fake_filesystem.create_dir("/test/subdir")
    fake_filesystem.create_file("/test/subdir/file2.py", contents="print('world')")
    fake_filesystem.create_dir("/test/subdir/nested")
    fake_filesystem.create_file("/test/subdir/nested/file3.py", contents="print('nested')")
...
def test_find_files_ignore_dirs(fake_filesystem, mock_args):
    ...
    fake_filesystem.create_file("/test/__pycache__/file2.pyc", contents="compiled")
    fake_filesystem.create_dir("/test/src")
    fake_filesystem.create_file("/test/src/file3.py", contents="print('src')")
...
def test_process_file_binary(mock_print, fake_filesystem, mock_args):
    ...
    # Create binary file
    fake_filesystem.create_file("/test/binary.bin", contents="\x00\x01\x02\x03")
...
def test_parse_args():
    ...
    # Test with flags
    with patch('sys.argv', ['catls.py', '/test/dir', '-r', '-a', '--debug', '-n']):
        args = parse_args()
