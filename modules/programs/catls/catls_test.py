...
def test_wildcard_to_regex():
    ...
    assert (
        wildcard_to_regex("test?.txt")
        == r"test.\.txt"
    )
    assert (
        wildcard_to_regex("file[abc].txt")
        == r"file\[abc\]\.txt"
    )
    assert (
        wildcard_to_regex("path/to/*.py")
        == r"path/to/.*\.py"
    )
...
def test_guess_filetype():
    ...
    assert (
        guess_filetype("test.js") == "javascript"
    )
...
def test_is_binary():
    ...
    with patch("subprocess.run") as mock_run:
        ...
        # Test binary file
        mock_result.stdout = (
            "test.bin: ELF 64-bit executable"
        )
        mock_run.return_value = mock_result
...
def test_should_include():
    ...
    assert (
        should_include("test.py", ["*.py"])
        is True
    )
    assert (
        should_include("test.js", ["*.py"])
        is False
    )
...
def test_should_ignore():
    ...
    assert (
        should_ignore("test.py", [], []) is False
    )
...
def test_find_files_basic(
    ...
    fake_filesystem.create_file(
        "/test/file2.txt", contents="text content"
    )
...
def test_find_files_hidden(
    fake_filesystem, mock_args
):
    """Test hidden file handling."""
    # Create test files including hidden ones
    fake_filesystem.create_file(
        "/test/visible.py", contents="visible"
    )
    fake_filesystem.create_file(
        "/test/.hidden.py", contents="hidden"
    )
...
def test_find_files_ignore_dirs(
    fake_filesystem, mock_args
):
    ...
    assert (
        "/test/__pycache__/file2.pyc" not in files
    )


@patch("builtins.print")
def test_process_file_binary(
    mock_print, fake_filesystem, mock_args
):
    ...
    with patch(
        "catls.is_binary", return_value=True
    ):
        process_file(
            "/test/binary.bin", mock_args
        )
...
def test_process_file_ignore(
    ...
    # Create test file
    fake_filesystem.create_file(
        "/test/ignored.pyc", contents="compiled"
    )

    mock_args.directory = "/test"
    mock_args.ignore_regex = [
        re.compile(r".*\.pyc")
    ]
