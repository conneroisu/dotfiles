...
class NixEdgeCaseTester:
    ...
    def test_malformed_nix_expressions(self):
        ...
        handled = len([r for r in self.results['malformed_expressions'] if r.get('handled', False)])
        print(f"   ✅ Handled {handled}/{len(malformed_expressions)} malformed expressions")
    ...
    def test_real_subprocess_failures(self):
        ...
        failure_scenarios = [
            {
                ...
                'method': lambda: subprocess.check_output(['nonexistent_command'], 
                                                        text=True, stderr=subprocess.PIPE)
            },
            {
                'name': 'Permission denied',
                'method': lambda: subprocess.check_output(['cat', '/etc/shadow'], 
                                                        text=True, stderr=subprocess.PIPE)
            },
            {
                'name': 'Timeout',
                'method': lambda: subprocess.check_output(['sleep', '10'], 
                                                        text=True, timeout=1)
            }
        ]
        ...
        handled = len([r for r in self.results['real_subprocess_failures'] if r.get('handled', False)])
        print(f"   ✅ Handled {handled}/{len(failure_scenarios)} subprocess failure scenarios")
    ...
    def test_flake_discovery_edge_cases(self):
        ...
        test_paths = [
            ...
            "",  # Empty path
            "./templates/go-shell" if Path("./templates/go-shell").exists() else "/tmp",  # Relative path
        ]
    ...
    def test_system_detection_accuracy(self):
        ...
        try:
            ...
            valid_format = "-" in detected_system and len(detected_system.split("-")) >= 2
    ...
    def test_file_operations_edge_cases(self):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            test_cases = [
                ...
                ('unicode.nix', '{ "🎉" = "unicode"; }'),  # Unicode content
                ('large.nix', '# ' + 'x' * 10000),  # Large file
                ('binary.dat', b'\x00\x01\x02\xff'),  # Binary content
            ]
